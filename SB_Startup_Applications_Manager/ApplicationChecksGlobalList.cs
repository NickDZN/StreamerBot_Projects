...
public class CPHInline
{
    public bool Execute()
    {
        ...
        try
        {
            ...
            {
                return Log(
                    logMessage: $"Failed to set the global variable. Error: {ex.Message}",
                    logLevel: 0,
                    halting: 1
                );
            }
            ...
            return Log(
                logMessage: $"Applications dictionary stored as a global variable with {applicationsToRun.Count} entries",
                logLevel: 2
            );
        }
        catch (Exception ex)
