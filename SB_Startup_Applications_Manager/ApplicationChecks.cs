...
public class CPHInline
{
    public bool Execute()
    {
        ...
        try
        {
            ...
            if (applicationListString == null || !applicationListString.Any())
                return Log(logMessage: "Application list is empty, or failed to parse", logLevel: 0, halting: 1);

            Log("Splitting application list string.", logLevel: 3);
            var applicationList = applicationListString
            ...
            foreach (var applicationPath in applicationList)
            {
                ...
                if (!IsProcessOpen(appName))
                {
                    ...
                    if (!OpenProcess(appPath, appExe))
                        Log($"Failed to open process: {appName}.", logLevel: 1);
                }
                else
                {
