...
public class LoadStartupConfigForm : Form
{
    ...
    LogControlDetails(applicationsGroupBox, "GroupBox");
    LogControlDetails(appTable, "TableLayoutPanel");
    foreach (Control control in appTable.Controls)
    {
        LogControlDetails(control, control.GetType().Name);
    }

    
    CPHLogger.LogI($"  Control Size: {applicationsGroupBox.Size}");
    CPHLogger.LogI($"  Control Dock: {applicationsGroupBox.Dock}, Anchor: {applicationsGroupBox.Anchor}");
    CPHLogger.LogI($"  Control Margin: {applicationsGroupBox.Margin}, Padding: {applicationsGroupBox.Padding}");

    CPHLogger.LogI($"  Control Size: {appTable.Size}");
    CPHLogger.LogI($"  Control Dock: {appTable.Dock}, Anchor: {appTable.Anchor}");
    CPHLogger.LogI($"  Control Margin: {appTable.Margin}, Padding: {appTable.Padding}");


    foreach (Control child in appTable.Controls)
{
    var row = appTable.GetRow(child);
    var column = appTable.GetColumn(child);
    //var cellBounds = appTable.GetCellBounds(row, column);

    CPHLogger.LogI($"[DEBUG] Control '{child.GetType().Name}' in Cell[{row},{column}]:");
    //CPHLogger.LogI($"  Allocated Cell Bounds: {cellBounds}");
    CPHLogger.LogI($"  Control Size: {child.Size}");
    CPHLogger.LogI($"  Control Dock: {child.Dock}, Anchor: {child.Anchor}");
    CPHLogger.LogI($"  Control Margin: {child.Margin}, Padding: {child.Padding}");
}
...
public class UIComponentFactory
{
        ...
        int defaultValue = 2)
    {
        ...
        CPHLogger.LogV($"NumericUpDown created: Width={numericUpDown.Width}, Height={numericUpDown.Height}, Minimum={numericUpDown.Minimum}, " +
    ...
    public static RadioButton CreateRadioButton(string text, bool autoSize = true, bool isChecked = false)
    {
        ...
        CPHLogger.LogV($"RadioButton created. Properties: Text=\"{radioButton.Text}\", AutoSize={radioButton.AutoSize}, Dock={radioButton.Dock}, " + 
        ...
        bool isEnabled = true)
    {
        ...
        CPHLogger.LogV($"Button created. Properties: Text=\"{btn.Text}\", Width={btn.Width}, Height={btn.Height}, Enabled={btn.Enabled}, " + 
        ...
        AnchorStyles anchor = AnchorStyles.Left | AnchorStyles.Right)
    {
        ...
        CPHLogger.LogV($"ListBox created: Width={(widthParam.HasValue ? widthParam.Value.ToString() : "Auto")}, " +
...
public static class LayoutLogger
{
    ...
    private static void LogControlHierarchy(Control control, int depth = 0)
    {
        ...
        controlCounter++;
        string controlDetails = $"{indent}[{controlCounter}] {prefix}Control: {control.GetType().Name}, Name: {control.Name ?? "Unnamed"}, " +
